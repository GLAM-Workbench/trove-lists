...
external_imports = ['jupyterlab', 'voila', 'voila-material @ git+https://github.com/GLAM-Workbench/voila-material.git', 'beautifulsoup4']
    ...
    if not nb.name.startswith('.') and not nb.name.startswith('Untitled'):
        ...
        for cell in nb_json['cells']:
            for line in cell['source']:
                if match := re.search(r'^\s*import ([a-zA-Z_0-9]+)(?! from)', line):
                    imports.append(match.group(1))
                elif match := re.search(r'^\s*from ([a-zA-Z_0-9]+)\.?[a-zA-Z_]* import [a-zA-Z_]+', line):
                    imports.append(match.group(1))
